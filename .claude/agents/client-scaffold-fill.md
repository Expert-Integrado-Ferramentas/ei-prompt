...
1. Para cada `<campo>` em `<dados_coletados>`: o atributo `arquivo` do próprio `<campo>` diz literalmente em qual arquivo dentro de `<cliente_path>` (Orquestrador.md, Qualifier.md, Scheduler.md, Protractor.md ou Follow-Up.md) o placeholder correspondente vive — roteie SEMPRE por esse atributo, nunca por inferência de nome de campo. `Read` esse arquivo específico, depois `Edit`/`Write` substituindo o placeholder correspondente por `valor` (ou deixando o marcador literal `[PENDENTE - informação não fornecida]` quando `pendente="true"`). Se um `<campo>` chegar sem o atributo `arquivo` (contrato quebrado do Passo 2), não adivinhe: pare e reporte a inconsistência no resumo final em vez de aplicar o valor em um arquivo incerto.
